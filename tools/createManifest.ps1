...
# Title
'CACHE MANIFEST' + [Environment]::NewLine | Out-File -Append -Encoding utf8 $pathToManifest
...
Get-ChildItem -Path ..\src\www\public -Recurse | Where-Object{!($_.PSIsContainer)} | % { $_.FullName.ToString() -replace($pathToRemoveString, '') -replace('\\', '/') | Out-File -Append -Encoding utf8 $pathToManifest }
...
'*' + [Environment]::NewLine | Out-File -Append -Encoding utf8 $pathToManifest

# A random string in order to update the appcache
$hash = .\createRandomString.ps1
'# Hash from Powershell script:' | Out-File -Append -Encoding utf8 $pathToManifest
$hash + [Environment]::NewLine | Out-File -Append -Encoding utf8 $pathToManifest
'# Hash from deployment:' | Out-File -Append -Encoding utf8 $pathToManifest
'# StringToBeReplacedAtDeployment' | Out-File -Append -Encoding utf8 $pathToManifest